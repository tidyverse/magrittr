...
*magrittr* also provides four related pipe operators. These are not as
...
The compound assignment pipe operator `%<>%` can be used as the first pipe
...
Finally, the compound assignment with exposition pipe operator `%<$>%`combines the previous two, as a shorthand for
expressions like `foo <- foo %$% bar`:
```{r, eval = FALSE}
