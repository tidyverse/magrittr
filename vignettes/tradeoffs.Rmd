...
Once the pipe has been transformed to regular R expressions, it must be evaluated. There are two primary options:

* In the __current environment__
* In the environment of a new function, a __closure environment__.
...
To discuss implementation challenges with a concrete example, we'll take the following values funtions:
...
The main drawback to this approach is that currently `eval_bare()` loses the visibility flag. This can be fixed, but needs work in C. 
