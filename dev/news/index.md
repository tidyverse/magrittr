...
- Fixed a C level protection issue in `%>%`
  ([\#256](https://github.com/tidyverse/magrittr/issues/256)).

## magrittr 2.0.2

CRAN release: 2022-01-26

- New eager pipe `%!>%` for sequential evaluation
  ([\#247](https://github.com/tidyverse/magrittr/issues/247)). Consider
  using [`force()`](https://rdrr.io/r/base/force.html) in your functions
  instead to make them strict, if sequentiality is required. See the
  examples in
  [`?"pipe-eager"`](https://magrittr.tidyverse.org/dev/reference/pipe-eager.md).

- Fixed an issue that could cause pipe invocations to fail in versions
  of R built with `--enable-strict-barrier`.
  ([\#239](https://github.com/tidyverse/magrittr/issues/239),
...
- Fixed issue caused by objects with certain names being present in the
  calling environment
  ([\#233](https://github.com/tidyverse/magrittr/issues/233)).

- Fixed regression in
  [`freduce()`](https://magrittr.tidyverse.org/dev/reference/freduce.md)
  with long lists (kcf-jackson/sketch#5).
...
([\#120](https://github.com/tidyverse/magrittr/issues/120)). The main
consequence of this change is that warnings and errors can now be
handled by trailing pipe calls:
...
In previous versions of magrittr, the behaviour of
[`return()`](https://rdrr.io/r/base/function.html) within pipe
expressions was undefined. Should it return from the current pipe
...
implementation, and so calling
[`return()`](https://rdrr.io/r/base/function.html) is now an error.
...
In magrittr 1.5, [`return()`](https://rdrr.io/r/base/function.html) used
to return from the current pipe expression. You can rewrite this to the
equivalent:
...
[`return()`](https://rdrr.io/r/base/function.html) calls as this is a
common occurrence in packages:
...
[`return()`](https://rdrr.io/r/base/function.html) in a pipeline, even
if trailing.
...
The solution to fix this is to call
[`force()`](https://rdrr.io/r/base/force.html) on the inputs to force
...
inspected by [`sys.frames()`](https://rdrr.io/r/base/sys.parent.html) or
[`sys.parent()`](https://rdrr.io/r/base/sys.parent.html). Using these
functions for implementing actual functionality (as opposed as debugging
tools) is likely to produce bugs. Instead, you should generally use
[`parent.frame()`](https://rdrr.io/r/base/sys.parent.html) which works
even when R code is called from non-inspectable frames. This happens
with e.g. [`do.call()`](https://rdrr.io/r/base/do.call.html) and the new
...
- Can now use the placeholder `.` with the splicing operator `!!!` from
  rlang ([\#191](https://github.com/tidyverse/magrittr/issues/191)).
...
- Piped arguments are now persistent. They can be evaluated after the
  ...
  factories ([\#159](https://github.com/tidyverse/magrittr/issues/159),
  [\#195](https://github.com/tidyverse/magrittr/issues/195)).
